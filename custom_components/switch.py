...
COILS_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(CONF_HUB, default=DEFAULT_HUB): cv.string,
        vol.Optional(CONF_VERIFY_COIL): cv.positive_int,
    }
)
...
class ModbusCoilSwitch(ToggleEntity, RestoreEntity):
    ...
    def __init__(self, hub, name, slave, coil, verify_coil):
        ...
        self._coil = int(coil)
        self._is_on = None
        self._verify_coil = verify_coil if verify_coil else self._coil
    ...
    def update(self):
        ...
        result = self._hub.read_coils(self._slave, self._verify_coil, 1)
        try:
